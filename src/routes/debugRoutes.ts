...
router.get('/simulate-flow', async (req, res) => {
  ...
  try {
    ...
    if (!user || !user.consumerProfile) {
      return res.status(404).json({ error: "Consumer user not found!" });
    }
    ...
    res.json({ success: true, message: "Topup and Gas recharge simulated successfully! Check logs/email." });
  } catch (err: any) {
